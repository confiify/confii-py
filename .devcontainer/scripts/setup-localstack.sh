...
aws --endpoint-url=$ENDPOINT s3 mb s3://confii-test 2>/dev/null || echo "  Bucket confii-test already exists"
aws --endpoint-url=$ENDPOINT s3 mb s3://confii-prod 2>/dev/null || echo "  Bucket confii-prod already exists"
...
api:
  ...
  timeout: 30
EOF
aws --endpoint-url=$ENDPOINT s3 cp /tmp/test-config.yaml s3://confii-test/config.yaml 2>/dev/null || true
