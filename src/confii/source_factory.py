...
def create_loader_from_config(source_dict: Dict[str, Any]) -> Any:
    ...
    Raises:
        ConfiiError: If the ``type`` is unknown or required keys are
            missing.
    """
    ...
    if source_type is None:
        raise ConfiiError("Source configuration must include a 'type' key")
    ...
    raise ConfiiError(f"Unknown source type: '{source_type}'")
