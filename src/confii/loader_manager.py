...
class LoaderManager:
    ...
    def load_plugins() -> Dict[str, Any]:
        ...
        Scans the ``confii.loaders`` entry-point group for installed
        ...
        try:
            eps = importlib.metadata.entry_points(group="confii.loaders")
        except TypeError:
            # Python < 3.10
            eps = importlib.metadata.entry_points().get("confii.loaders", [])
