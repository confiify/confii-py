...
from confii.config_diff import ConfigDiff, ConfigDiffer, ConfigDriftDetector
from confii.config_introspection import (
    get_all_keys,
    get_nested_value,
    get_schema_info,
    has_key,
...
class ConfigAccess:
    ...
    def _set_internal(self, key_path: str, value: Any, override: bool = True) -> None:
        ...
        if self.env_config:
            ...
            for key in keys[:-1]:
                if key not in env_current or not isinstance(env_current[key], dict):
                    env_current[key] = {}
                env_current = env_current[key]
            env_current[final_key] = value

            from confii.utils.dict_utils import set_nested
    ...
    def generate_docs(self, format: str = "markdown") -> str:
        ...
        for row in rows:
            ...
            if has_pydantic:
                ...
                lines.append(
                    f"| `{row['key']}` | {row['type']} | {val_str} | {row['source']} |"
                )
