"""Command-line interface for Confii.

This module provides the CLI commands for Confii, including
...
try:
    ...
    raise ImportError(
        "click is required for the CLI. Install with: pip install confii[cli]"
    )

from confii.config import Config
from confii.plugin_loader import get_loader
...
def get_examples_info():
    ...
    examples = {
        "working_demo.py": {
            "description": "Comprehensive demo of core Confii features",
...
@click.group()
def cli():
    """Confii CLI - Configuration management made easy"""
    pass
...
def export(env, loader_specs, format, output):
    ...
    Example:
        >>> confii export production --loader yaml:config.yaml --format=json --output=config.json
    """
    try:
        config = create_config(env, loader_specs, False, True, True)
...
def debug(env, loader_specs, key, export_report):
    ...
    Example:
        >>> confii debug production --key=database.host --loader yaml:config.yaml
        >>> confii debug production --export-report=debug.json
    """
    try:
...
def lint(env, loader_specs, fix, strict):
    ...
    Example:
        ...
        >>> confii lint production --loader yaml:config.yaml --strict
    """
    try:
        config = create_config(env, loader_specs, False, True, True)
...
def migrate(source_format, config_file, output, target_format):
    """Migrate configuration from other tools to Confii format.

    This command helps migrate configurations from other tools (python-dotenv,
    Dynaconf, Hydra) to Confii format. Preserves configuration values
...
def explain(env, loader_specs, key):
    ...
    Example:
        >>> confii explain production --key=database.host --loader yaml:config.yaml
    """
    try:
        loaders = parse_loader_specs(loader_specs) if loader_specs else None
        config = Config(
        ...
        if key:
            ...
            click.echo("Example: confii explain production --key=database.host")
...
def docs(env, loader_specs, fmt, output):
    ...
    Example:
        ...
        >>> confii docs production --loader yaml:config.yaml --format json --output config-reference.md
    """
    try:
        config = create_config(env, loader_specs, False, True, True)
...
@cli.group()
def examples():
    """Manage and explore Confii examples"""
    pass
...
def list_examples(verbose):
    ...
    if not examples_info:
        ...
        click.echo("Visit https://github.com/confiify/confii-py/tree/main/examples")
        return

    click.echo("🚀 Available Confii Examples\n")
    click.echo("=" * 60)
    ...
    click.echo("  confii examples export <name> [--output-dir <path>]")
    click.echo("\nTo export all examples:")
    click.echo("  confii examples export --all")
...
def export_examples(name, export_all, output_dir):
    ...
    for example_name in to_export:
        ...
        if dest_file.exists() and not click.confirm(
