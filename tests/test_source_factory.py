# pyright: basic
"""Tests for confii.source_factory module."""
...
from confii.source_factory import (
    _expand_env_vars,
...
class TestHTTPLoader:
    @patch("confii.loaders.remote_loader.HAS_REQUESTS", True)
    @patch("confii.loaders.remote_loader.requests", create=True)
...
class TestCustomLoader:
    def test_custom_loader_dynamic_import(self):
        loader = create_loader_from_config(
            {
                "type": "custom",
                "class": "confii.loaders.yaml_loader.YamlLoader",
    ...
    def test_custom_loader_no_args(self):
        loader = create_loader_from_config(
            {
                "type": "custom",
                "class": "confii.loaders.env_file_loader.EnvFileLoader",
...
class TestErrorCases:
    def test_unknown_type_raises_error(self):
        with pytest.raises(ConfiiError, match="Unknown source type: 'foobar'"):
            create_loader_from_config({"type": "foobar"})

    def test_missing_type_raises_error(self):
        with pytest.raises(ConfiiError, match="must include a 'type' key"):
