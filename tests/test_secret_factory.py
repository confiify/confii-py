...
from confii.exceptions import ConfiiError
...
class TestCreateVaultAuth:
    ...
    def test_token_missing_raises(self):
        with pytest.raises(ConfiiError, match="token"):
            _create_vault_auth({"method": "token"})

    def test_approle(self):
        with patch(
            "confii.secret_factory.AppRoleAuth",
            create=True,
        ) as mock_cls:
    ...
    def test_unknown_method_raises(self):
        with pytest.raises(ConfiiError, match="Unknown vault auth method"):
...
class TestCreateSecretStore:
    ...
    @patch(
        "confii.secret_stores.providers.aws_secrets_manager.BOTO3_AVAILABLE",
        True,
    )
    @patch(
        "confii.secret_stores.providers.aws_secrets_manager.boto3",
        create=True,
    )
    ...
    @patch("confii.secret_stores.providers.hashicorp_vault.HVAC_AVAILABLE", True)
    @patch("confii.secret_stores.providers.hashicorp_vault.hvac", create=True)
    ...
    @patch("confii.secret_stores.providers.azure_key_vault.AZURE_AVAILABLE", True)
    @patch(
        "confii.secret_stores.providers.azure_key_vault.SecretClient",
        create=True,
    )
    @patch(
        "confii.secret_stores.providers.azure_key_vault.DefaultAzureCredential",
        create=True,
    )
    ...
    def test_gcp_secret_manager_provider(self, mock_sm):
        ...
        from confii.secret_stores.providers.gcp_secret_manager import (
            GCPSecretManager,
        )
    ...
    def test_multi_provider_empty_stores_raises(self):
        with pytest.raises(ConfiiError, match="at least one store"):
    ...
    def test_unknown_provider_raises(self):
        with pytest.raises(ConfiiError, match="Unknown secret store provider"):
...
class TestCreateSecretResolverFromConfig:
    ...
    def test_missing_store_raises(self):
        with pytest.raises(ConfiiError, match="store"):
