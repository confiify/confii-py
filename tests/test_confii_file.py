"""Tests for confii self-configuration file support.

Verifies that confii can read its own settings from dedicated
confii.yaml/.json/.toml files, falling back to pyproject.toml.
"""
...
from confii.config_reader import (
    _DEFAULT_SETTINGS,
    clear_config_cache,
...
# ---------------------------------------------------------------------------
# Tests: read_confii_file
# ---------------------------------------------------------------------------


class TestReadConfiiFile:
    """Test loading from various confii.* file formats."""

    def test_load_yaml(self, tmp_path):
        data = {"default_environment": "staging", "deep_merge": False}
        _write_yaml(tmp_path / "confii.yaml", data)
    ...
    def test_load_yml(self, tmp_path):
        ...
        _write_yaml(tmp_path / "confii.yml", data)
    ...
    def test_load_json(self, tmp_path):
        ...
        _write_json(tmp_path / "confii.json", data)
    ...
    def test_load_toml(self, tmp_path):
        ...
        _write_toml(tmp_path / "confii.toml", data)
    ...
    def test_non_hidden_takes_priority_over_hidden(self, tmp_path):
        ...
        result = read_confii_file(search_dir=str(tmp_path))
        # confii.json is searched before .confii.yaml
        assert result["default_environment"] == "non-hidden"

    def test_malformed_file_is_skipped(self, tmp_path):
        # Write invalid YAML
        bad_file = tmp_path / "confii.yaml"
        bad_file.write_text(":\n  bad: [unterminated")

        # Provide a valid fallback
        _write_json(tmp_path / "confii.json", {"default_environment": "fallback"})
...
class TestConfigInitIntegration:
    """Test that Config() picks up settings from confii.* files."""

    def test_config_reads_from_confii_yaml(self, tmp_path, monkeypatch):
        """Config should apply settings from confii.yaml."""
        _write_yaml(
            tmp_path / "confii.yaml",
            {
                "default_environment": "staging",
    ...
    def test_explicit_params_override_file(self, tmp_path, monkeypatch):
        """Explicit Config() params should override confii.yaml values."""
        _write_yaml(
            tmp_path / "confii.yaml",
            {
                "default_environment": "staging",
    ...
    def test_config_reads_from_json_file(self, tmp_path, monkeypatch):
        """Config should apply settings from confii.json."""
        _write_json(
            tmp_path / "confii.json",
            {
    ...
    def test_config_reads_from_toml_file(self, tmp_path, monkeypatch):
        """Config should apply settings from confii.toml."""
        _write_toml(
            tmp_path / "confii.toml",
            {
    ...
    def test_pyproject_fallback_when_no_config_file(self, tmp_path, monkeypatch):
        """When no confii.* file exists, fall back to pyproject.toml."""
        pyproject = tmp_path / "pyproject.toml"
        pyproject.write_text(
            textwrap.dedent(
                """\
    ...
    def test_env_switcher_from_config_file(self, tmp_path, monkeypatch):
        """env_switcher from confii.yaml should be applied."""
        _write_yaml(
            tmp_path / "confii.yaml",
            {
